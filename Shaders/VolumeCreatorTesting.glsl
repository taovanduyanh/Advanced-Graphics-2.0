...
void main() {
    ...
    dSSBO[gl_GlobalInvocationID.x * nearHalfIcoNormals.length() + gl_GlobalInvocationID.y][0] = 1.0 / 0.0;
    dSSBO[gl_GlobalInvocationID.x * nearHalfIcoNormals.length() + gl_GlobalInvocationID.y][1] = -1.0 / 0.0;
    ...
    for (uint i = 0; i < 3; ++i) {
        ...
        dSSBO[gl_GlobalInvocationID.x * nearHalfIcoNormals.length() + gl_GlobalInvocationID.y][0] = min(d, dSSBO[gl_GlobalInvocationID.x * nearHalfIcoNormals.length() + gl_GlobalInvocationID.y][0]);
        dSSBO[gl_GlobalInvocationID.x * nearHalfIcoNormals.length() + gl_GlobalInvocationID.y][1] = max(d, dSSBO[gl_GlobalInvocationID.x * nearHalfIcoNormals.length() + gl_GlobalInvocationID.y][1]);
    }
