...
Shader::Shader(string cFile) {
	program = glCreateProgram();
	objects[0] = GenerateShader(cFile, GL_COMPUTE_SHADER);
	objects[1] = 0;
	objects[2] = 0;

	glAttachShader(program, objects[0]);
}
...
Shader::~Shader() {
	if (objects[1]) {
		...
		glDetachShader(program, objects[0]);
		glDeleteShader(objects[0]);
	}
